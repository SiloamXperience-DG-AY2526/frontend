...
export default function Projects() {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                ...
                setError(null);
                const data = await getVolunteerProjects();

                setProjects(data);

                const statuses = data.reduce((acc: Record<string, ProjectApprovalStatus>, project: Project) => {
    ...
    const handleStatusChange = async (projectId: string, newStatus: ProjectApprovalStatus) => {
        ...
        try {
            ...
            setProjectStatuses(prev => ({
                ...prev,
                [projectId]: projects.find(p => p.id === projectId)?.approvalStatus || prev[projectId],
            }));
        } finally {
            setLoading(null);
        }
    ...
    return (
        <>
            ...
            {/* Table */}
            {projects.length === 0 ? (
                <div className="text-center py-12 border border-[#195D4B] rounded-xl bg-white">
                            ...
                            {projects.map((project) => (
                                ...
                                >
                                    <td className="px-4 py-4">
                                        <Link href={`/projects/${project.id}/feedback`} className="hover:underline">
