...
export default function FeedbackAnalyticsPage() {
  ...
  const responseRate =
    totalShown > 0 ? Math.round((totalResponse / totalShown) * 100) : 0;

  const averageScore =
    respondedFeedback.length > 0
      ? Math.round(
          (respondedFeedback.reduce(
            (sum, f) => sum + (f.score as number),
            0
          ) /
            respondedFeedback.length) *
            20
        )
      : 0;
      ...
      <main className="flex-1 p-4 md:p-6 lg:p-8">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#56E0C2] mx-auto" />
